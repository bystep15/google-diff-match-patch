...
class DiffMatchPatch {
  ...
  List<Diff> diff_main(String text1, String text2,
    ...
    if (deadline === null) {
      ...
      if (Diff_Timeout <= 0) {
        ...
        deadline = deadline.add(new Duration(365, 0, 0, 0, 0));
      } else {
        deadline = deadline.add(new Duration(0, 0, 0, 0,
                                             (Diff_Timeout * 1000).toInt()));
      }
    }
   ...
   * Returns a List of Diff objects.
   */
  List<Diff> _diff_compute(String text1, String text2,
                           bool checklines, Date deadline) {
   ...
   * Returns a List of Diff objects.
   */
  List<Diff> _diff_lineMode(String text1, String text2, Date deadline) {
    ...
    while (pointer < diffs.length) {
      ...
      case DIFF_EQUAL:
        ...
        if (count_delete >= 1 && count_insert >= 1) {
          ...
          final a = diff_main(text_delete, text_insert, false, deadline);
   ...
   * Returns a List of Diff objects.
   */
  List<Diff> _diff_bisect(String text1, String text2, Date deadline) {
    // Cache the text lengths to prevent multiple calls.
    final text1_length = text1.length;
    final text2_length = text2.length;
    final max_d = ((text1_length + text2_length + 1) / 2).toInt();
    ...
    for (int d = 0; d < max_d; d++) {
      ...
      if ((new Date.now()).compareTo(deadline) == 1) {
        break;
      }
   ...
   * Returns a List of Diff objects.
   */
  List<Diff> _diff_bisectSplit(String text1, String text2,
                               int x, int y, Date deadline) {
  ...
  String _diff_linesToCharsMunge(String text, List<String> lineArray,
    ...
    while (lineEnd < text.length - 1) {
      ...
      if (lineHash.containsKey(line)) {
        chars.add(new String.fromCharCodes([lineHash[line]]));
      } else {
        lineArray.add(line);
        lineHash[line] = lineArray.length - 1;
        chars.add(new String.fromCharCodes([lineArray.length - 1]));
      }
    }
  ...
  void _diff_charsToLines(List<Diff> diffs, List<String> lineArray) {
    ...
    for (Diff diff in diffs) {
      for (int y = 0; y < diff.text.length; y++) {
        text.add(lineArray[diff.text.charCodeAt(y)]);
      }
  ...
  int diff_commonPrefix(String text1, String text2) {
    ...
    final n = Math.min(text1.length, text2.length);
    for (int i = 0; i < n; i++) {
      if (text1[i] != text2[i]) {
        return i;
      }
  ...
  int diff_commonSuffix(String text1, String text2) {
    ...
    final text1_length = text1.length;
    final text2_length = text2.length;
    final n = Math.min(text1_length, text2_length);
    for (int i = 1; i <= n; i++) {
   ...
   */
   int _diff_commonOverlap(String text1, String text2) {
    ...
    final text_length = Math.min(text1_length, text2_length);
  ...
  List<String> _diff_halfMatch(String text1, String text2) {
    ...
    final hm1 = _diff_halfMatchI(longtext, shorttext,
        ((longtext.length + 3) / 4).ceil().toInt());
    // Check again based on the third quarter.
    final hm2 = _diff_halfMatchI(longtext, shorttext,
  ...
  RegExp nonAlphaNumericRegex_ = const RegExp(@"[^a-zA-Z0-9]");
  RegExp whitespaceRegex_ = const RegExp(@"\s");
  RegExp linebreakRegex_ = const RegExp(@"[\r\n]");
  RegExp blanklineEndRegex_ = const RegExp(@"\n\r?\n$");
  RegExp blanklineStartRegex_ = const RegExp(@"^\r?\n\r?\n");
  ...
  void diff_cleanupMerge(List<Diff> diffs) {
    diffs.addLast(new Diff(DIFF_EQUAL, ''));  // Add a dummy entry at the end.
    int pointer = 0;
    int count_delete = 0;
    int count_insert = 0;
    String text_delete = '';
  ...
  String diff_prettyHtml(List<Diff> diffs) {
    ...
    for (Diff aDiff in diffs) {
      String text = aDiff.text.replaceAll('&', '&amp;').replaceAll('<', '&lt;')
  ...
  String diff_toDelta(List<Diff> diffs) {
    ...
    for (Diff aDiff in diffs) {
      switch (aDiff.operation) {
      case DIFF_INSERT:
        text.add('+').add(encodeURI(aDiff.text)).add('\t');
        break;
      case DIFF_DELETE:
        text.add('-').add(aDiff.text.length).add('\t');
        break;
      case DIFF_EQUAL:
        text.add('=').add(aDiff.text.length).add('\t');
        break;
  ...
  int _match_bitap(String text, String pattern, int loc) {
    ...
    if (best_loc != -1) {
      score_threshold = Math.min(_match_bitapScore(0, best_loc, loc, pattern),
          score_threshold);
  ...
  void _patch_addContext(Patch patch, String text) {
    ...
    // Add the prefix.
    final prefix = text.substring(Math.max(0, patch.start2 - padding),
        patch.start2);
    if (!prefix.isEmpty()) {
      ...
      while (!bigpatch.diffs.isEmpty()) {
        ...
        while (!bigpatch.diffs.isEmpty()
          ...
          if (diff_type == DIFF_INSERT) {
            ...
            patch.diffs.addLast(bigpatch.diffs[0]);
  ...
  List<Patch> patch_fromText(String textline) {
    ...
    while (textPointer < text.length) {
      ...
      final patch = new Patch();
      patches.add(patch);
      patch.start1 = Math.parseInt(m.group(1));
      if (m.group(2).isEmpty()) {
        ...
        patch.start1--;
        patch.length1 = Math.parseInt(m.group(2));
      }

      patch.start2 = Math.parseInt(m.group(3));
      if (m.group(4).isEmpty()) {
        ...
        patch.start2--;
        patch.length2 = Math.parseInt(m.group(4));
      }
      textPointer++;
